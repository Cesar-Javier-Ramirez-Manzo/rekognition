...
class SearchFaces extends React.Component {
    ...
    detectEach = async (moreBoxes) =>{
        ...
        this.setState({facesMatches: arrayNames})
    ...
    drawBox = async (boxes) => {
        ...
        for (var i = 0; i < boxes.length; i++) {
            ...
            ctx.drawImage(image, x1, y1, x2, y2, moveNext, 10, 100, 100);
            moveNext += 102
        }
    ...
    uploadImage = async () => {
        ...
        await axios.post("https://t5ggb319a1.execute-api.us-east-2.amazonaws.com/buscar/imagen", imageData)
            .then(response => {
                console.log(response.data)
                //this.setState({ facesMatches: response.data['names'] })
        ...
        return (
            <>
                <Container className="menu">
                    ...
                    <br></br>
                    <h3 className="title">Personas encontradas</h3>
                    <Table striped bordered hover id="tabla" className="tabla">
                        <thead>Nombres</thead>
                        <tbody>
